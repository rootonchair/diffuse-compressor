...
def run_model_cli() -> None:
    ...
    output = "outputs/checkpoints/svdq-int4_r32-sana-1.6b.safetensors"
    parser = default_arg_parser(
        "Lawrence-cj/Sana_1600M_1024px_BF16_diffusers_ch5632",
        output,
        steps=20,
        guidance_scale=4.5,
        batch_size=256,
    ...
    if args.output == output:
        args.output = f"outputs/checkpoints/svdq-{args.precision}_r32-sana-1.6b.safetensors"
    cache_dir = args.cache_dir or "outputs/calibration/sana-1.6b"
    pipe = load_pipeline(
        "SanaPipeline",
        args.model_id,
        device=args.device,
    ...
    target_config = sana_target_config(args.precision)
...
def sana_target_config(precision: Precision = "int4") -> TargetConfig:
    ...
    return TargetConfig(
        ...
        targets=[
            ...
            TargetRule(modules=["transformer_blocks.*.ff.conv_inverted"], export_name="transformer_blocks.{0}.mlp_fc1", kind="conv"),
            TargetRule(modules=["transformer_blocks.*.ff.conv_point"], export_name="transformer_blocks.{0}.mlp_fc2", kind="conv"),
        ],
    )
