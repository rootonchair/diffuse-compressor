...
class QuantizationLogger:
    ...
    def __init__(
        self, name: str | py_logging.Logger | LoggingConfig = _PACKAGE_LOGGER_NAME, config: LoggingConfig | None = None
    ) -> None:
    ...
    def enabled(self) -> bool:
        return (
            self.config is not None and self.config.enabled and (self.config.text_output or self.config.target_records)
        )
